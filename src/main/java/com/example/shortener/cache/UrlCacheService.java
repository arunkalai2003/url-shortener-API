...
public class UrlCacheService {
    ...
    public UrlCacheService(StringRedisTemplate redis, ShortenerProperties p, Clock clock, ObjectMapper mapper) {
        this.redis = redis;
        this.p = p;
        this.clock = clock;
        this.mapper = mapper;
    ...
    public Optional<CachedUrl> get(String code) {
        ...
        try {
            return Optional.of(mapper.readValue(v, CachedUrl.class));
    ...
    private Optional<CachedUrl> getFallback(String code, Throwable t) {
        return Optional.empty();
    }
    ...
    public void put(String code, CachedUrl value) {
        ...
        if (value.expiresAt() != null) {
            Duration until = Duration.between(clock.instant(), value.expiresAt());
            if (until.isNegative() || until.isZero()) return;
            if (until.compareTo(ttl) < 0) ttl = until;
        }
        try {
            redis.opsForValue().set(key(code), mapper.writeValueAsString(value), ttl);
    ...
    public void putNegative(String code) {
        redis.opsForValue().set(key(code), NOT_FOUND, p.negativeCacheTtl());
    }
