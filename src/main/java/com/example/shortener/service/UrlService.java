...
public class UrlService {
    ...
    public CreateShortUrlResponse create(CreateShortUrlRequest request, String idempotencyKey) {
        ...
        String requestHash = HashingUtil.sha256(normalized + "|" + String.valueOf(request.customAlias()) + "|" + String.valueOf(expiresAt) + "|" + String.valueOf(request.campaign()));

        if (idempotencyKey != null && !idempotencyKey.isBlank()) {
            var prior = idempotencyRepo.findById(idempotencyKey);
            if (prior.isPresent()) {
                if (!prior.get().getRequestHash().equals(requestHash))
        ...
        String fingerprint = HashingUtil.sha256(normalized); // campaign-independent canonical identity
    ...
    private void validateAlias(String a) {
        ...
        if (p.reservedAliases().stream().anyMatch(x -> x.equalsIgnoreCase(a)))
    ...
    private Object load(String code) {
        ...
        if (db.isEmpty()) {
            redisCache.putNegative(code);
            return "NOT_FOUND";
        }
        CachedUrl c = toCached(db.get());
        redisCache.put(code, c);
        return c;
    }
    ...
    private ResolvedUrl validateCached(String code, CachedUrl c) {
        ...
        if (c.expiresAt() != null && !c.expiresAt().isAfter(clock.instant()))
    ...
    public void disable(String code) {
        ...
        e.setStatus(UrlStatus.DELETED);
        repo.save(e);
        l1.invalidate(code);
        redisCache.evict(code);
    }

    private void cache(ShortUrlEntity e) {
        CachedUrl c = toCached(e);
        l1.put(e.getShortCode(), c);
        redisCache.put(e.getShortCode(), c);
    }

    private CachedUrl toCached(ShortUrlEntity e) {
        return new CachedUrl(e.getNormalizedUrl(), e.getExpiresAt(), e.getStatus().name());
    }

    private CreateShortUrlResponse response(ShortUrlEntity e) {
        return new CreateShortUrlResponse(e.getShortCode(), p.baseUrl() + "/" + e.getShortCode(), e.getNormalizedUrl(), e.getCreatedAt(), e.getExpiresAt());
    }
